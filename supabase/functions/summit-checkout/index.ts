...
Deno.serve(async (req) => {
  ...
  const { data: pedido, error: dbErr } = await db
    ...
    .insert({
      ...
      lote: lote.nome, valor: lote.avista, quantidade: 1,
    })
    .select("id")
    .single();
  if (dbErr || !pedido)
  ...
  const sucesso = `${SITE}/obrigado.html?lote=${encodeURIComponent(lote.nome)}&valor=${lote.avista}`;
  const pref = {
    items: [{
      id: "summit-2026",
      title: `Ingresso Intercâmbio Summit 2026 — ${lote.nome}`,
      description: "11 de novembro de 2026 · Contentix, Av. Paulista 967, São Paulo",
      category_id: "tickets",
      quantity: 1,
      currency_id: "BRL",
      unit_price: lote.avista,
    }],
    payer: {
      name: nome,
      surname: sobrenome,
    ...
    metadata: { origem: "site-summit-2026" },
  };
